...
proc freq data=manu.uniqhosp_20240712;
tables pallcare_days_1_3;
run;

proc freq data=manu.uniqhosp_20240712 ;
tables (cvshock_ICU_newoccur_hosp respfail_ICU_newoccur_hosp renalfail_ICU_newoccur_hosp 
coagfail_ICU_newoccur_hosp liverfail_ICU_newoccur_hosp any_organ_fail
pallcare_ICU_newoccur_hosp pallcare_days_1_3 pallcare_anyICUday1_11_hosp)*perci / out=freqs;
run;

ods excel file='filepath' ;
proc tabulate data=manu.uniqhosp_20240712 missing order=freq
format =8.2;
class  perci cvshock_ICU_newoccur_hosp respfail_ICU_newoccur_hosp renalfail_ICU_newoccur_hosp 
coagfail_ICU_newoccur_hosp liverfail_ICU_newoccur_hosp pallcare_ICU_newoccur_hosp pallcare_days_1_3;

table cvshock_ICU_newoccur_hosp respfail_ICU_newoccur_hosp renalfail_ICU_newoccur_hosp 
coagfail_ICU_newoccur_hosp liverfail_ICU_newoccur_hosp pallcare_ICU_newoccur_hosp pallcare_days_1_3
all ='Column Total',
all(colpctn) perci*(n rowpctn) / nocellmerge;
run;
ods excel close;
...
*caterpillar plot;
proc sgplot data=manu.estimates_meanpop;
...
proc rank data=manu.estimates_meanpop/*_20240626*/ out=quintile_m2 groups=5;
var mu;
ranks quintile;
run;
...
proc sql;
select model2_quintile, count(distinct patienticn) as pts, count(distinct unique_hosp_count_id) as hosps, count(distinct sta6a) as sta6a
from manu.uniqhosp_20240712
group by model2_quintile;
quit;
proc sql;
select count(distinct patienticn) as pts, count(distinct unique_hosp_count_id) as hosps, count(distinct sta6a) as sta6a
from manu.uniqhosp_20240712;
quit;

				
/*/*/* Model 2 Quintiles: Figure 3 */*/*/
...
*7/10/24 - create any day palliative care variable (received pall care on any ICU day 1 through 11);
...
data chart_model2;
set manu.uniqhosp_20240712;
if any_organ_fail = 0 then any_organ_fail = .;
run;

ods excel file='filepath'
options(sheet_name = "Any" sheet_interval='proc');
proc freq data=chart_model2;
tables (any_organ_fail 
		pallcare_anyICUday1_11_hosp anyday_pallcare_perci)*model2_quintile 
		/	nopercent nocol norow missing;
run;

*input raw data to create plot;

*any pall care;
data anychart_model2;
input model2_quintile any_organ_fail pallcare_anyICUday1_11_hosp;
datalines;
0 1380 3353
1 2724 3870
2 5185 7047
...
*any;
data anychart2_model2;
set anychart_model2;
length group $50.;
format value 4.1;
group = "Any new late-onset organ failure"; value = (any_organ_fail/19877)*100; output;
group = "Palliative care consult"; value = (pallcare_anyICUday1_11_hosp/26586)*100; output;
drop any_organ_fail pallcare_anyICUday1_11_hosp ;
run;
...
*any pall care;
ods graphics / width=8.5in height=5in;
proc sgplot data=anychart2_model2 ;
styleattrs datacolors=(cx0a75a1 cx0e89bd cx0e9ed6 cx85badf cxb6d0eb);
vbar  group / response = value group=model2_quintile groupdisplay=cluster 
				/*datalabel*/ datalabelattrs=(size=12) /*outlineattrs=(color=black)*/ name="bar" nooutline barwidth=0.8;
format model2_quintile pquint.;
keylegend "bar"/ title=""  valueattrs=(size=14) titleattrs=(size=14) noborder ;
yaxis label="Hospitalizations (%)" labelattrs=(size=14) valueattrs=(size=14) values=(0 to 35 by 5) grid ;
xaxis display=(nolabel) labelattrs=(size=14) valueattrs=(size=14);
run;
